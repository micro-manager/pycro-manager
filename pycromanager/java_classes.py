...
class _CoreCallback:
    ...
    def _callback_recieving_fn(self, bridge_port, core_callback):
        callback_java = JavaObject('org.micromanager.remote.RemoteCoreCallback',
                                   args=(Core(port=bridge_port),))
...
class Core(_JavaObjectShadow):
    ...
    """
    def __new__(cls, convert_camel_case=True, port=Bridge.DEFAULT_PORT, new_socket=False, debug=False):
        """
    ...
    def get_core_callback(self, callback_fn=None, bridge_port=Bridge.DEFAULT_PORT):
        ...
        :param callback_fn: a function that takes (name, *args)
        :param bridge_port:
        :return:
        """
...
class Magellan(_JavaObjectShadow):
    ...
    def __new__(cls, convert_camel_case=True, port=Bridge.DEFAULT_PORT, new_socket=False, debug=False):
        ...
        return bridge._construct_java_object("org.micromanager.magellan.api.MagellanAPI", new_socket=new_socket)
...
class JavaObject(_JavaObjectShadow):
    ...
    """
    def __new__(cls, classpath, args: list=None, port=Bridge.DEFAULT_PORT, new_socket=False,
                convert_camel_case=True, debug=False):
        """
        :param classpath: str
...
class JavaClass(_JavaObjectShadow):
    ...
    def __new__(cls, classpath,  port=Bridge.DEFAULT_PORT, new_socket=False, convert_camel_case=True, debug=False):
        """
