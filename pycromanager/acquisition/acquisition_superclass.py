...
class EventQueue(Queue):
    """
    A queue that can hold both event_implementations/lists of event_implementations and generators of event_implementations/lists of event_implementations. When a generator is
...
class Acquisition(metaclass=Meta):
    ...
    def __init__(
        ...
        image_process_fn : Callable
            ...
            metadata, queue), where queue is a Queue object that holds upcoming acquisition event_implementations. The function
            should return either an (image, metadata) tuple or a list of such tuples
        event_generation_hook_fn : Callable
            hook function that will as soon as acquisition event_implementations are generated (before hardware sequencing optimization
            in the acquisition engine. This is useful if one wants to modify acquisition event_implementations that they didn't generate
    ...
    def mark_finished(self):
        """
        Signal to acquisition that no more event_implementations will be added and it is time to initiate shutdown.
        This is only needed if the context manager (i.e. "with Acquisition...") is not used.
        """
        # Some acquisition types (e.g. ExploreAcquisitions) generate their own event_implementations
        # and don't send event_implementations over a port
        if self._event_queue is not None:
            # this should shut down storage_implementations and viewer as appropriate
            self._event_queue.put(None)

    def acquire(self, event_or_events: dict or list or Generator) -> AcquisitionFuture:
        """
        Submit an event or a list of event_implementations for acquisition. A single event is a python dictionary
        with a specific structure. The acquisition engine will determine if multiple event_implementations can
        be merged into a hardware sequence and executed at once without computer-hardware communication in
        between. This sequencing will only take place for event_implementations that are within a single call to acquire,
        ...
        event_or_events  : list, dict, Generator
            A single acquistion event (a dict), a list of acquisition event_implementations, or a generator that yields
            acquisition event_implementations.

        """
        try:
            if self._are_events_finished():
                raise AcqAlreadyCompleteException(
                    'Cannot submit more event_implementations because this acquisition is already finished')
            ...
            if isinstance(event_or_events, GeneratorType):
                ...
                def notifying_generator(original_generator):
                    ...
                    # it doesn't needlessly track event_implementations
    ...
    def abort(self, exception=None):
        """
        Cancel any pending event_implementations and shut down immediately
        ...
        # Clear any pending event_implementations on the python side, if applicable
        if self._event_queue is not None:
            self._event_queue.clear()
            # Don't send any more event_implementations. The event sending thread should know shut itself down by
    ...
    def _are_events_finished(self):
        """
        Check if all event_implementations have been processed and executed
        """
        pass
    ...
    def _create_event_queue(self):
        """Create thread safe queue for event_implementations so they can be passed from multiple processes"""
...
def _validate_acq_events(events: dict or list):
    """
    Validate if supplied event_implementations are a dictionary or a list of dictionaries
    that contain valid event_implementations. Throw an exception if not
    ...
    if isinstance(events, dict):
        ...
        if len(events) == 0:
            raise Exception('event_implementations list cannot be empty')
        for event in events:
            if isinstance(event, dict):
                _validate_acq_dict(event)
            else:
                raise Exception('event_implementations must be a dictionary or a list of dictionaries')
    else:
        raise Exception('event_implementations must be a dictionary or a list of dictionaries')
...
def multi_d_acquisition_events(
    ...
    # collect all event_implementations into a single list
    base_event = {"axes": {}}
    events = []
