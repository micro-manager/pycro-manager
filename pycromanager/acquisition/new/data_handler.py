import threading
import queue
from typing import Any, Dict, Tuple, Callable, Union, Iterable, Sequence, Optional
import numpy as np
...
class DataHandler:
    """
    Object that handles acquired data while it is waiting to be saved. This object is thread safe and handles
    ...
    def __init__(self, storage: DataStorageAPI,
                 process_function: Callable[[DataCoordinates, np.ndarray, JsonValue],
                                Optional[Union[DataCoordinates, np.ndarray, JsonValue,
                                               Sequence[DataCoordinates, np.ndarray, JsonValue]]]] = None):
        self._storage = storage
        self._process_function = process_function
        self._intake_queue = _PeekableQueue()
        # locks for synchronizing access the queues/dicts
        self._intake_lock = threading.Lock()
        self._data_metadata_future: Dict[Any, Tuple[np.ndarray, Dict, Optional[AcquisitionFuture]]] = {}
    ...
    def _run_intake_thread(self):
        while True:
            if self._process_function:
                ...
                data, metadata, _ = self._data_metadata_future[coordinates]
                ...
                with self._intake_lock:
                    with self._processed_lock:
                        # Move from intake queue to processed or saving queue as appropriate
                        coordinates = self._intake_queue.get()
                        # discard old data/metadata because it has now been processed
                        _, _, future = self._data_metadata_future.pop(coordinates)
                        if processed is None:
                            ...
                            self._data_metadata_future[coordinates] = (data, metadata, future)
                            future._notify_data(coordinates, data, metadata, processed=True, saved=False)
                        else:  # multiple items
    ...
    def _transfer_to_storage(self):
        ...
        """
        # lock = self._processed_lock if self._process_function else self._intake_lock
    ...
    def get(self, coordinates: DataCoordinates, return_data=True, return_metadata=True
        ...
        """
        data_metadata_future = self._data_metadata_future.get(coordinates, None)
        data, metadata = None, None
        if data_metadata_future:
            data, metadata, future = data_metadata_future
        else:
    ...
    def put(self, coordinates: Any, image: np.ndarray, metadata: Dict, acquisition_future: Optional[AcquisitionFuture]):
        ...
        with self._intake_lock:
            ...
            self._data_metadata_future[coordinates] = (image, metadata, acquisition_future)
        if acquisition_future:
            acquisition_future._notify_data(coordinates, image, metadata, processed=False, saved=False)
